                            ...
                            <div class="card-body">
                                <table id="example" class="display nowrap" style="width:100%;text-align:center;">
                                    ...
                                    <tbody>
                                        ...
                                        @endphp
                                        @if(isset($employees))
                                        @foreach($employees as $showdata)
                                      
                                      <tr id="tr-{{$showdata->invoice_id}}">
                                        <td data-toggle="tooltip" title="SL">{{$sl++}}</td>
                                        <td data-toggle="tooltip" title="Order ID">{{ $showdata->name }}</td>
                                        <td data-toggle="tooltip" title="Order ID">{{ $showdata->designation }}</td>
                                        <td data-toggle="tooltip" title="Order ID">{{ $showdata->phone }}</td>
                                        <td data-toggle="tooltip" title="Order ID">{{ $showdata->email }}</td>
                                        ...
                                        <td>
                                          <div class="row">
                                             <a href="{{route('employee.edit', $showdata->id)}}" class="btn btn-outline-warning btn-sm mr-2" target="_blank" style="float: left;">Edit</a>

                                            <form action="{{route('employee.destroy', $showdata->id)}}" method="post">
                                                @csrf
                                                @method('DELETE')
